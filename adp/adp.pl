...
B<adp.pl> - Run from within B<OPUS>.  This is the second step of a three step B<OPUS> pipeline.  The first step is B<adpst.pl> and the last step is B<adpfin.pl>.

=head1 DESCRIPTION

I<adp.pl> is the script which runs the main part of the adp B<OPUS> pipeline.  This is the second step of the three step ADP pipeline.

The purpose of this step is to properly process each type of ADP file received.  The first step is to create a directory named the same as the file in the WORKDIR location referenced in the I<adp.resource> and move the file from ADP_INPUT (see I<adp.resource>) to this new directory.  Then, depending on the file, process it according to the list of ACTIONS below.
...
This is the directory which the alert mangement daemon will monitor for adp pipeline alerts.

=item B<PARFILES>

This is the location of the saved .par files for the different executables. Make sure that you clean this directory out if you need to rerun an orbita file.
...
#	061211 - Jake - should use ISDCLIB::Error instead of die (or DoOrDie)
#`$mymkdir -p $newworkdir`;
#die "*******     ERROR:  cannot mkdir $newworkdir." if ($?);
&ISDCLIB::DoOrDie ( "$mymkdir -p $newworkdir" );
...
foreach my $dir ( "$orgdir/ref", "$adpdir", "$ENV{ARC_TRIG}", "$ENV{ALERTS}" ) {
	#	061211 - Jake - should use ISDCLIB::Error instead of die
	#	`$mymkdir -p $dir`;
	#	die "*******     ERROR:  cannot mkdir $dir." if ($?);
	&ISDCLIB::DoOrDie ( "$mymkdir -p $dir" );
}	#	060119 - Jake - Cleaner and smaller and a 4 sets of duped code

=head1 ACTIONS

Note that the patterns below are Perl regular expressions.  Documentation on these can be found in Programming Perl, Second Edition, Larry Wall et al, Page 57.

In all cases below the environment variable AUXDIR is used to reference to top level of the Auxiliary repository.  This variable is set in the I<adp.resource> file and that references an entry in the I<adp.path> file.
...
This is a pad file which has a 2 digit AO number xx in the file name.  The file is copied both to the org/AOxx as well as the aux/AOxx directory.

=item B<iop_([0-9]{2}).*>

This is an iop file which has a 2 digit AO number in the file name.  The file is copied both to the org/AOxx as well as the aux/AOxx directory.

=item B<ocs_([0-9]{2}).*>

This is a ocs file which has a 2 digit AO number in the file name.  The file is copied both to the org/AOxx as well as the aux/AOxx directory.
...
=cut
	...
	if ($dataset =~ /(ocs|pad|iop)_([0-9]{2}).*/) {	#	iop's may have a trailing _
		...
		&ISDCPipeline::PipelineStep (
			...
			"program_name" => "$mychmod -w $newworkdir/$dataset",
			);			       
		...
		open(AIT,">$temptrigger") 
			or &ISDCLIB::Error ( "cannot open $temptrigger to write!" );
...
This is a pod file which has a 4 digit revolution number rrrr in the file name.  The file is copied both to the org/rrrr as well as the aux/rrrr.000 directory.
...
This is an opp file which has a 4 digit revolution number rrrr in the file name.  First the file is untarred and oconvertopp is called for each one of the .*opp_nnnnnnnn.txt files stored in the .tar archive.  This results in a boxplan_nnnnnnnn.fits and shotplan_nnnnnnnn.fits file for each .txt file.  Then the opp file is copied both to the org/rrrr and all of the boxplan and shotplan files are copied to the aux/rrrr.000 directory.
...
=cut
	...
	if ($dataset =~ /opp_([0-9]{4})_\d{4}_(\d{4}).*/) {
		...
		&ISDCPipeline::PipelineStep (
			"step"         => "adp - write protect files",
			"program_name" => "$mychmod -w $newworkdir/$dataset $newworkdir/*fits",
			);			       
...
This is a PAF file which has a 4 digit revolution number rrrr in the file name.  convertattitude is run on this file and the resulting attitude file is copied to aux/rrrr.000.  The original PAF file is copied to org/rrrr.
...
=cut
	...
	if ($dataset =~ /([0-9]{4})_([0-9]{2})\.PAF/) {
		...
		&ISDCPipeline::PipelineStep (
			"step"         => "adp - write protect files",
			"program_name" => "$mychmod -w $newworkdir/$dataset $newworkdir/*/*fits*",		# 050302 - Jake - SPR 3772 - unlikely gzipped
			);			       
...
This is either a ASF or an AHF file which has a 4 digit revolution rrrr number in the file name.  convertattitude is run on this file and the resulting attitude file is copied to aux/rrrr.000.  The original file is copied to org/rrrr if it is an AHF file or org/rrrr/asf if it is an ASF file.  Missing from this step is cnrtattupdate which should be run but has yet to be delivered.
...
This is the orbita named orbita_YYYYMMDD.  First convertorbit is run producing a list of directories which this orbita file contained data for.  Then all of the resulting orbit*.fits files are copied from the just created rrrr directories into the cooresponding adp/rrrr.000 directories in the adp repository.  Finally the orbita file is copied to org/ref.  NB: convertorbit stores the last orbit processed for both predicted and historic in the .par file.  Therefore if you try to rerun the same orbita file twice, even if the have different file names, the second run will not produce anything.
...
=cut
	...
	if ($dataset =~ /orbita.*/) {
		...
		&ISDCPipeline::PipelineStep (
			"step"         => "adp - write protect files",
			"program_name" => "$mychmod -w $newworkdir/*/orbit*fits* $newworkdir/$dataset",		# 050302 - Jake - SPR 3772 - unlikely gzipped
			"stoponerror"  => 0,
			);			       
		...
		&ISDCLIB::DoOrDie ( "$mymkdir -p $adpdir/$lastrev.000/logs/" ) if (!(-d "$adpdir/$lastrev.000/logs/"));
...
This is the revno file named to revno_YYYYMMDD.  Convertrevolution is run to produce a revolution_yyyymmdd.fits file and this is copied into adp/ref/revno.  For current software compatibility a soft link is made to the file adp/ref/revolution.fits but in the future this should go away.  Once all of this is done then revno_YYYYMMDD file is copied to org/ref/revno.
...
=cut
	...
	if ($dataset =~ /revno_(.*)/) {
		...
		if (!(-e $dirname)) {
			...
			&ISDCLIB::DoOrDie ( "$mymkdir -p $dirname" );
		}
		...
		&ISDCPipeline::PipelineStep (
			"step"         => "adp - write protect files",
			"program_name" => "$mychmod -w $newworkdir/*.fits* $newworkdir/$dataset ",		# 050302 - Jake - SPR 3772 - unlikely gzipped
			);			       
		...
		open(AIT,">$temptrigger") or &ISDCLIB::Error ( "cannot open $temptrigger to write!" );
...
This is a TSF file which has a 4 digit revolution number rrrr followed by a 4 digit version number vvvv in the file name.  The first step is to run convertprogram.  Then createpdef is run on the resulting timeline_summary file and using the same version of the POD file in the repository to create a corresponding pointing_definition_predicted_vvvv.fits file with the same version number.  NB: Currently the pod version number MUST agree with the TSF version number.  Since it is possible that alerts were generated the alerts are copied, then the TSF file is copied to the org/rrrr directory.  Then the timeline_summary_vvvv.fits file and the pointing_definition_vvvv.fits files are both copyied to the adp/rrrr.000 directory.
...
=cut
	...
	if ($dataset =~ /TSF_([0-9]{4})_.*([0-9]{4})\.INT/) {
		...
		&ISDCPipeline::PipelineStep (
			"step"         => "adp - write protect files",
			"program_name" => "$mychmod -w $newworkdir/*fits* $newworkdir/*/*fits* $newworkdir/$dataset",		# 050302 - Jake - SPR 3772 - unlikely gzipped
			);			       
	...
	if ($dataset =~ /THF_(\d{6})_\d{4}/) {
		...
		chdir "$newworkdir" or &ISDCLIB::Error ( "cannot chdir to $newworkdir" );
		...
		open(THF,"$dataset") or &ISDCLIB::Error ( "cannot open $dataset to read" );
		...
		if (!($copy)) {
			...
			chdir "$ENV{WORKDIR}" or &ISDCLIB::Error ( "cannot chdir $ENV{OPUS_WORK}" );
			...
			&ISDCLIB::Error ( "cannot remove $ENV{WORKDIR}/$ENV{OSF_DATASET}:  @result" ) if ($retval);
			...
			chdir "$ENV{LOG_FILES}" or &ISDCLIB::Error ( "cannot chdir $ENV{LOG_FILES}" );
			unlink "${dataset}_log.txt" or &ISDCLIB::Error ( "cannot delete log ${dataset}_log.txt" );
			unlink "$ENV{OSF_DATASET}.log" or &ISDCLIB::Error ( "cannot delete log $ENV{OSF_DATASET}.log" );
		...
		if (! -d "$orgpath") {
			...
			&ISDCLIB::DoOrDie ( "$mymkdir -p $orgpath" );
		}
		&ISDCPipeline::PipelineStep (
			"step"         => "adp - write protect files",
			"program_name" => "$mychmod -w $newworkdir/$dataset",
...
This is a OLF file.  It is not possible to derive the proper directory from the file name since the revolution number is not contained in the file name.  The first step is to run convertprogram which will create a rev directory rrrr which is the rev this OLF file is for.  Then copy the time_correlation.fits, moc_out_of_limits.fits, the observation_log.fits from the aux/rrrr.000 directory if they exist and then rerun convertprogram.  This way the existing files are updated.  Once this is done get the latest pod for this revolution and run createpdef.  Since alerts could have been generated copy the alerts to the alerts directory, copy the OLF file to the org/rrrr/olf directory, and move the log file which was previously put in the wrong place by I<adpst.pl> because it is not possible to derive the rev number rrrr from the file name to the right place in org/rrrr/olf.  Finally copy all of the resulting *.fits files to the proper adp/rrrr.000 directory.
...
=cut
	...
	if ($dataset =~ /^(\d{9})\.OLF/) {
		...
		if (!-s "$dataset") {
			...
			chdir "$ENV{WORKDIR}" or &ISDCLIB::Error ( "Cannot chdir to $ENV{WORKDIR}" );
			...
			&ISDCLIB::Error ( "Cannot remove $ENV{WORKDIR}/$ENV{OSF_DATASET}:\n@result" ) if ($retval);
			...
			&ISDCLIB::Error ( "Cannot remove $ENV{LOG_FILES}/${dataset}_log.txt $ENV{LOG_FILES}/$opuslink:\n@result" ) if ($retval);
		...
		##################################
			...
			if (@timecors) {
				...
				rename "$newworkdir/$rev/$oldtimecor.fits","$newworkdir/$rev/time_correlation_${timestamp}_${vers}.fits" 
					or &ISDCLIB::Error ( "Cannot rename $newworkdir/$rev/$oldtimecor.fits to $newworkdir/$rev/time_correlation_${timestamp}_${vers}.fits" );
					#	061211 - Jake - should use ISDCLIB::Error instead of die
			...
			&ISDCLIB::DoOrDie ( "$mymkdir -p $adpdir/$rev.000/logs/olf" ) if (!(-d "$adpdir/$rev.000/logs/olf"));
...
sub RevArcCheck {
	...
	if ($retval) {
		...
		&ISDCLIB::Error ( "Cannot run \'osf_test -p adp.path -pr dataset\':  @osfs" );
	}
	else {
		my $osf;
		foreach $osf (@osfs) {
	...
	&ISDCLIB::Error ( "Cannot find OLFs in org:  $orgdir/$revnum/olf/" ) unless (@olfs);
	...
	for ($i=$start;$i<=$end;$i++) {
		...
		if (!$olfosfs{"${i}_OLF"}) {
			...
			&ISDCLIB::Error ( "I'm confused, as there appear to be more than one OSF matching ${i}_OLF:  @result" ) if ($#result > 0);
			if (@result) {
...
#	&UnixLIB::Gzip ( glob ( "$adpdir/$revnum.000/*.fits" ) );
	...
	open(AIT,">$ENV{ARC_TRIG}/aux_${revnum}0000.trigger_temp") 
		or &ISDCLIB::Error ( "cannot create trigger file $ENV{ARC_TRIG}/aux_${revnum}0000.trigger_temp" ); 
#		or die "*******        ERROR:  cannot create trigger file $ENV{ARC_TRIG}/aux_${revnum}0000.trigger_temp"; 
	...
	&ISDCLIB::Error ( "cannot create trigger file $ENV{ARC_TRIG}/aux_${revnum}0000.trigger" ) if ($retval);
...
subroutine to check the contents of a revolution and ensure that no junk files remain before the entire directory is blindly archived.  This function will exit with an error if a junk file is found.
...
Once all of this is done remove the scratch directory which held all of the work files.

=head1 RESOURCE FILE

The resource file I<adp.resource> contains all the environment variables for B<adp.pl>.  All of these reference items in the I<adp.path> path file.

=head1 PATH FILE ENTRIES USED

Path file entries are hard coded directories and will have to be manually edited to move the pipeline to another set of directories.  There are two sets of directories in the currently delivered path files.  The first start with /isdc/nrt and these are to point to ISDC repositories.  Make them point to the right repositories.  The second currently point to /isdc/scratch and this is a work area for opus to work in.  /isdc/scratch is not the most pleasing name so possibly another should be chosen.

=over 5

=item B<rii_aux>

This points to the current top of the auxiliary repository.  This is assigned to the B<AUXDIR> environment variable by B<OPUS>.

=item B<adp_work>

This points to a scratch working directory for the B<ADP> pipeline.  This is assigned to the B<WORKDIR> environment variable by B<OPUS>.

=item B<alerts>

This points to a scratch directory to hold alerts for the alert managment daemon. Assigned to B<ALERTS> by B<OPUS>.

=item B<parfiles>

This points to a scratch directory to hold par files.  Assigned to the B<PARFILES> environment variable by B<OPUS>

=back

=head1 RESTRICTIONS

Any changes in the input file names will require changes in this script since it matches on the file names.  Note that corresponding changes will also have to be made in I<adpst.pl>.

=head1 REFERENCES

For further information on B<adpst.pl> or B<adpfin.pl> please run perldoc on those files, ie, C<perldoc adpst.pl>.
