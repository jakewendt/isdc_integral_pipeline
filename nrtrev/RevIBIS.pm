...
#		$work_root = $root;
#		$work_root =~ s/.*ibis_(.*)_veto.*/working_index_$1/;
...
#		if (-e "$prev_index") {
		...
		if (-e "$this_index") {
#		if ( ( -e "$this_index" ) || ( -e "$this_index.gz" ) ) {		#	060420
			&ISDCPipeline::FindIndex (
				"index"    => "$this_index",
		...
		&ISDCPipeline::RunProgram ( "$mymv ${work_root}_this.fits ${work_root}_selected.fits" );	#051118 - Jake - added the last ;
...
#	if (! -d "$ENV{SCWDIR}/$revno/rev.000/aca/") {
#		`$mymkdir -p $ENV{SCWDIR}/$revno/rev.000/aca/`;
#		die "*******     ERROR:  cannot create archive trigger directory $ENV{SCWDIR}/$revno/rev.000/aca/\n" 
#			if ($?);
#	}
	&ISDCLIB::DoOrDie ( "$mymkdir -p $ENV{SCWDIR}/$revno/rev.000/aca/" ) unless ( -d "$ENV{SCWDIR}/$revno/rev.000/aca/" );
	
	# Loop over all MODULES (0-15):
	for ($module=0;$module<=15;$module++) {
		...
		if (!-e "$workdir/aca/$veto_results") {
			&ISDCPipeline::PipelineStep (
				...
				"par_template" => "IBIS-VETO-CAL.tpl",
				"subdir"       => "$workdir/aca",
				);    
		}
		$outdol = "aca/".$veto_results."[IBIS-VETO-CAL]" ;
		
		# TO BE FIXED:  email from Tony Bird 17 Dec 2002 requests specifying
		#  inArchive="" so as not to take into account old "suspect" results
		($retval,@results) = &ISDCPipeline::PipelineStep (
			"step"           => "$proc - IBIS ibis_aca_veto for module $module",
			"program_name"   => "ibis_aca_veto",
			"par_inVETO"     => "working_index_raw_selected.fits[GROUPING]",
			#"par_inArchive" => "working_index_aca_selected.fits[GROUPING]",
			"par_inArchive"  => "",
			"par_outVCAL"    => "$outdol",
			"par_lastScW"    => "",
			"par_module"     => "$module",
		...
		if ($retval) {
			if ( $retval == 152062 ) {
				&Message ( "WARNING:  no valid data for module $module;  continuing." );
				next;
			}
			else {
				print "*******     ERROR:  return status of $retval from ibis_aca_veto is not allowed\n";
				exit 1;
			}
		}
	} # end of for modules 0-15
	
	#  SPR 2708:  check to see if it did anything;  if not, delete
	#	040701 - Jake - SPR 3725
...
sub ACAirvCU {
	...
	foreach $root ("idx/ibis_aca_cu_index","idx/ibis_raw_veto_index") {
		...
		#  Root name of the working indices (various versions):
#		$work_root = $root;
#		$work_root =~ s/.*ibis_(.*)_index/working_index_$1/;
...
#	if (! -d "$ENV{SCWDIR}/$revno/rev.000/aca/") {
#		`$mymkdir -p $ENV{SCWDIR}/$revno/rev.000/aca/`;
#		die "*******     ERROR:  cannot create archive trigger directory "
#			."$ENV{SCWDIR}/$revno/rev.000/aca/\n" if ($?);
#	}
	&ISDCLIB::DoOrDie ( "$mymkdir -p $ENV{SCWDIR}/$revno/rev.000/aca/" ) unless (  -d "$ENV{SCWDIR}/$revno/rev.000/aca/");
...
sub ICAirn {
	...
	if (-e "$isgr_context_index") {	#	060420
#	if ( ( -e "$isgr_context_index" ) || ( -e "$isgr_context_index.gz" ) ) {
		#  Why was this Collect before? 
		&ISDCPipeline::FindIndex (
		...
		#    &Error("No ISGRI context index found in $isgr_context_index\n") 
		#	unless (-e "$isgr_context_index");	#	040820 - Jake - SCREW 1533
	...
	if (-e "$ENV{OUTPATH}/scw/$revno/rev.000/idx/isgri_prp_noise_index.fits") {
		...
		|| ( -e "$prevrevdir/idx/isgri_prp_noise_index.fits.gz" ) ) {									#	060517 - Jake - these could be gzip'd ??
	...
	if (-e "$ENV{OUTPATH}/scw/$revno/rev.000/idx/isgri_pxlswtch_index.fits") {
		...
		|| ( -e "$prevrevdir/idx/isgri_pxlswtch_index.fits.gz" ) ) {		#	060420
...
sub ACAirc {			#	SCREW 791
	...
	#	040331 - Jake - A single time for the entire revolution may be a problem.
	#	I choose to use $revstart to begin with.  Will try another if there are problems.
	my $revtime  = $revstart;
	...
	my $isgrdropmod = &ISDCPipeline::GetICFile (
		...
		"select"    => "( VSTART <= $revtime ) && (VSTOP >= $revtime)",
		);	#	050406 - Jake - SCREW 1499
	&Error ( "No IC file ISGR-DROP-MOD found." ) unless ( $isgrdropmod );		#	050406 - Jake - SCREW 1499
	...
	&ISDCPipeline::PipelineStep (	#	VERSION 3.0 will not work here
		...
		"par_chatter"    => "2",
		"par_pathBipar"  => "",			
		"par_icDOL"      => "$isgrdropmod",			#	050504 - Jake - SCREW 1499 - added for ibis_isgr_cal_energy-3.3
		"par_GODOL"      => "$isgroffsmod",			#	050504 - Jake - SCREW 1499 - added for ibis_isgr_cal_energy-3.3
	...
	if ( -r $isgricdtecor ) {		#	May want to change this to check if the DOL structure exists
		#
...
#		} elsif (-e "$prevrevfiledir/idx/isgri_pxlswtch_index.fits") {
			...
			|| (-e "$prevrevfiledir/idx/isgri_pxlswtch_index.fits.gz") ) {		#	060420
		...
		if ($working_pxlswtch_index) {
			...
			$working_pxlswtch_index = "working_pxlswtch_index.fits"."[GROUPING]";
				&ISDCPipeline::PipelineStep (
					...
					"par_inGRP"        => "",
					"par_outGRP"       => "$isgricdtecor_DOL",							#	may not need to be a DOL
					"par_isgrRawEvts"  => "",
					"par_isgrPrpEvts"  => "",
					"par_isgrCorEvts"  => "",
					...
					"par_probShot"     => "0.001",		#	050128 - Jake - added for ibis_isgr_evts_tag 3.0 from 2.3
															#	050316 - Jake - SCREW 1628 - from 0.1 to 0.001
					);
		} else {
...
#	my $prevrevdir = &ISDCLIB::FindDirVers ( "$ENV{SCWDIR}/$prevrev/rev" );
	...
	&ISDCPipeline::PipelineStep (
		...
		"par_OBT_TYPE"        => "",			#	Jake added 040109 SPR 3411 - default = "" 
		);
	...
	if ( $retval ) {			#	if $retval is NOT ISDC_OK
		if (`$myls $workdir/*alert* 2> /dev/null`) {
			...
			&ISDCPipeline::PipelineStep (
				...
				"par_ScWIndex"   => "$ENV{REP_BASE_PROD}/idx/scw/GNRL-SCWG-GRP-IDX.fits[1]",	#	070112 - Jake - 4644
				"subdir"         => "$workdir",
				);
		}
...
sub ICAidp {
	...
	if (-e "$workdir/cfg/picsit_context_$stamp.fits") {
		...
		&ISDCPipeline::FindIndex (
			"index"    => "$ENV{OUTPATH}/idx/scw/GNRL-SCWG-GRP-IDX.fits",				
			"select"   => "OBTEND > \'$diff\' && REVOL==$revno",	#	040920 - Jake - SPR 3842
			"workname" => "working_prpscws_index.fits",
			"sort"     => "OBTEND",
			"subdir"   => "$workdir",
			"required" => 0,
...
#		my @flt_lists;
		...
		@flt_lists = sort(glob($prevrevdir."/cfg/picsit_fault_list*.fits")) unless (@flt_lists);									#	060517 - Jake - these could be gzip'd ??
		
		my $flt_list = $flt_lists[$#flt_lists]."[PICS-FALT-STA]" if (@flt_lists);
		
		# then this will run with that file and create a new version in current
		#  working subdir/cfg
...
#		die "*******     ERROR:  cannot check blackboard for other ilts:\n@result" if ($retval);
...
#					die "*******     ERROR:  cannot parse $_";
			...
			#  Just a consistency check.  Must have versions starting from 1.
#			die "*******     ERROR:  found ".scalar(keys(%ilts))." ILTs on blackboard, "
#				."but last version is $lastvers;  something's wrong here!" 
		...
		#   odd behavior.  
		if ( $iltvers > 1 ) {	#	SCREW 1824
#		if ($iltvers >= 9) {
			#  Just count the raw veto files to give a number to the alert
...
#			or die "*******     ERROR:  cannot open log to write at "
#				."$ENV{SCWDIR}/$revno/rev.000/logs/ibis_ilt_${stamp}_00_log.txt";
...
#	my $prevrevdir = &ISDCLIB::FindDirVers ( "$ENV{SCWDIR}/$prevrev/rev" );
	...
	if (!defined($ENV{OSF_DCF_NUM})) {
		#  this can happen if you run opus_wrapper on an ILT file;  fudge it
		$iltvers = "1";
	}
...
#	`$mymkdir -p $workdir/raw`;
#	&Error ( "Cannot create new working dir $workdir/raw!\n" ) if ($?);	
...
#	`$mymkdir -p $workdir/cfg`;
#	&Error ( "Cannot create new working dir $workdir/cfg!\n" ) if ($?);
	...
	# Get current rev's noise maps, whatever exits.  
	...
	&ISDCPipeline::FindIndex (
		...
		"sort"     => "OBTEND",
		"subdir"   => "$workdir",
		"required" => 0,#  don't error if no match
		) if (-e "$ENV{OUTPATH}/idx/scw/GNRL-SCWG-GRP-IDX.fits");				
	...
	#  LUTs file:  TO BE FIXED, currently an ASCII file
	...
	if ( -e "$ENV{OUTPATH}/idx/rev/ISGR-DEAD-CFG-IDX.fits" ) {
		&ISDCPipeline::FindIndex (				#	040322 - Jake - I don't completely understand why I use this, but ...
			"index"    => "$ENV{OUTPATH}/idx/rev/ISGR-DEAD-CFG-IDX.fits",
			"select"   => "REVOL < $revno",	#	Need to use this because can rerun data, but must make sure that the REVOL has actually been set
			"workname" => "working_isgridead_index.fits",
			"subdir"   => "$workdir",
			#	"sort" => "OBTEND",											#	DO I NEED?
			#	"required" => 0,		#  don't error if no match		#	DO I NEED?
			);																	
	...
	($retval, @result) = &ISDCPipeline::PipelineStep (
		...
		"par_ratioEffic"   => "$ENV{ILT_RATIO_EFFIC}",# 0.7
		"par_revolWait"    => "$ENV{ILT_REVOL_WAIT}",# 3
		"par_whichLUT1"    => "2",# requires IC tree >= 4.50
		...
		"stoponerror"      => "0",	#	041116 - Jake - should have been here for the error check below, but we've never run this
		);

	my $DoNotAttachIndex = "";	#	040412 - Jake - can be a list of items to not index, but will only be this one
	if ( $retval ) {			#	if $retval is NOT ISDC_OK
		if (  ( $retval == 144910 )	#	060310 - removed 144909
			|| ( $retval == 144911 ) ) {
			...
			&Error ( "Return status of $retval from ibis_isgr_low_thres is not allowed" ); #051118 - Jake - added the ;
		}
	}
...
#	die "*******     ERROR:  Cannot parse TPF names like $result[0]" unless ($date);
...
#	`$mymkdir -p $ENV{TPF_DIR}` unless (-d "$ENV{TPF_DIR}");
#	&Error ( "*******     ERROR:  cannot create new working dir $ENV{TPF_DIR}!\n" ) if ($?);	
...
#	`$mymkdir -p $ENV{IFTS_OUTBOX}` unless (-d "$ENV{IFTS_OUTBOX}");
#	&Error ( "*******     ERROR:  cannot create new working dir $ENV{IFTS_OUTBOX}!\n" ) if ($?);
	...
	if (defined($ENV{ILT_ALERT})) {
		...
		#	Hello Mathias,
		#	Following your suggestion, we can assign:
		#	alert 3045, level 2: "TPF vX SENT for rev RRRR dump YYYYMMDDHHMMSS"
		#	when an IBIS context file has been created AND sent to the MOC.
		#	The procedure for the operators would be to check in the IFTS log 
		#	file that the file has been correctly sent to MOC.
		#	Operators, could you add this procedure in the list of alerts?
		#	Thanks, Nami 
...
#
#	Pre-SCREW 1569 Alert
#		&ISDCPipeline::WriteAlert(
#			"step" => "$proc - sending ILT alert",
#			"message" => "TPF v$iltvers for rev $revno dump $stamp created;  check IFTS outbox.tmp",
#			"subdir" => "$workdir",
#			"level" => $ENV{ILT_ALERT},
#			"id" => "3000",
#			);
...
#	my $prevrevdir = &ISDCLIB::FindDirVers ( "$ENV{SCWDIR}/$prevrev/rev" );
	...
	# Don't delete but save for ACA PICsIT ?
	#  unlink(glob("$workdir/working*"));
...
#	=item B<ACAprc{NRT,CONS}>
#	
#	The OBT_ACQ attribute is read from the input data file and science windows 
#	within approximately one day are selected using B<idx_find>.  The executable 
#	B<ibis_aca_pics> then takes this index, the PICS-ENER-MOD IC file, the input data 
#	file, and creates a new calibration file.  That result is write protected and indexed.  
#	
#	In the NRT case, this is done for individual calibration spectra, while in the 
#	Consolidated case, all for the revolution are collected in an index and processed at once.  
#	
