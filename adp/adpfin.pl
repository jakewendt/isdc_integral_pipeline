...
I<adpfin.pl> - Run from within B<OPUS>.  This is the last step of a three stage pipeline which does Auxiliary Data Preparation.  The first step is B<adpst.pl> and the second step is B<adp.pl>.

=head1 DESCRIPTION

I<adpfin.pl> - Run from within B<OPUS>.  This is the last step of the three step ADP pipeline.  The purpose of this step is write the closing log entries and then exit.  If it turns out that additional cleanup needs to be done for the B<ADP> pipeline then these steps could be put here.
...
The resource file for I<adpfin.pl> contains all the environment variables which are used from the path file for the B<ADP> pipeline.
...
For further information on B<adpst.pl> or B<adp.pl> please run perldoc on those files, ie, C<perldoc adpst.pl>.
