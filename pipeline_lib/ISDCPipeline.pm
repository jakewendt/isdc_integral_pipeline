...
#use strict;	#	use strict will cause unnecessary compilation failures
use Carp;
use File::Basename;
use Cwd;					#	Why not just `pwd`?
use File::Copy;		#	WHY use File::Copy::copy?  Why not just $mycp?
use UnixLIB;
use TimeLIB;
use ISDCLIB;
use OPUSLIB;
...
sub PipelineStart {
	&Carp::croak ( "PipelineStart: Need even number of args" ) if ( @_ % 2 );
...
sub PipelineFinish {
	&Carp::croak ( "PipelineFinish: Need even number of args" ) if ( @_ % 2 );
...
sub PipelineStep {
	&Carp::croak ( "PipelineStep: Need even number of args" ) if ( @_ % 2 );
	my %att = @_;
#
#	If I make these changes here, I suspect there will be many changes which 
#	will be needed elsewhere to undo this change. - Jake - SCREW 1437
#	my $initialcommonlogfile = $ENV{COMMONLOGFILE};		#	040408 - Jake - SCREW 1437
	...
	######################################################
	# $ENV{OSF_DATA_ID} is, or can be, undefined in the conssa unit tests 
#	$att{type} = "$ENV{OSF_DATA_ID}" unless ($att{type});		#	<-- line 311

	#	SPR 4071 - In order to avoid the error message:
	#	Use of uninitialized value in string at pipeline_lib/ISDCPipeline.pm line 311.
...
#	$ENV{PARFILES} = $att{pfiles} if defined ( $att{pfiles} );



	
	#  SCREW 1099 requires always parsing output.  But for functions like
	#   GetICFile, don't want that, so can still set it to zero.
	...
	&Error ( "No program name" ) unless ($att{program_name});	#	060525
	...
	if (($att{type} eq "scw") && ($att{dcf} eq "000")) {
		...
		&Error ( "Can't find science window $att{dataset}" );	#	060525
	}
	...
	if ($att{subdir}) {
		chdir $att{subdir} or 
		&Error ( "Can't change to subdirectory $att{subdir}" );	#	060525
	}
	
	# Set the path and PFILES
	# get current version of pfiles, extract other system pfiles (i.e. ftools)
	#  for appending
	#    print "========     Setting PFILES to OPUS path PARFILES;ISDC_ENV/pfiles\n";
	&Error ( "PARFILES not set!" ) unless (defined($ENV{PARFILES}));	#	060525
	$ENV{PFILES} = "$ENV{PARFILES};$att{environment}/pfiles";
	#$ENV{PFILES} = $ENV{PARFILES}.";".$att{environment}."/pfiles";
	#    print "========     Checking for LHEASOFT pfiles as well\n";
	#  $ENV{PFILES} .= ":"."$ENV{LHEASOFT}/syspfiles" if ($ENV{LHEASOFT});
	print "========     PFILES is now $ENV{PFILES}\n";
	# REMOVED;  the account set up should be correct;  only the pfiles changes.
	#    $ENV{PATH} = $att{environment}."/bin:".$ENV{PATH};
	#  PipelineEnvVars();
	# find all the parameters and build the command line
...
sub MakeIndex {
	...
	&Carp::croak ( "MakeIndex: Need even number of args" ) if ( @_ % 2 );
...
sub IndexDetach {
	&Carp::croak ( "IndexDetach: Need even number of args" ) if ( @_ % 2 );
...
sub CleanGroup {
	&Carp::croak ( "CleanGroup: Need even number of args" ) if ( @_ % 2 );
...
sub CollectIndex {
	...
	&Carp::croak ( "CollectIndex: Need even number of args" ) if ( @_ % 2 );
...
sub FindIndex {
	...
	&Carp::croak ( "FindIndex: Need even number of args" ) if ( @_ % 2 );
...
sub WriteAlert {
	...
	&Carp::croak ( "WriteAlert: Need even number of args" ) if ( @_ % 2 );
...
sub BBUpdate {
	...
	&Carp::croak ( "BBUpdate:  Need even number of args" ) if ( @_ % 2 );
