...
CONS_SSA_INPUT_DIR="${ISDC_SITE}/run/pipelines/cons/consssa/input"
SWG_INDEX_DIR="${REP_BASE_PROD}/idx/scw/"
CONS_SSA_BLACKLIST="${ISDC_SITE}/run/pipelines/cons/opus/cons_ssa_blacklist.txt"
...
cons_ssa_revno() {
   ...
   dal_dump_cmd="dal_dump inDol=${sub_index}+1 column=SWID outFormat=0"
   fdump_cmd="fdump prhead=no showrow=no showcol=no page=no ${sub_index}+1 STDOUT MEMBER_LOCATION -"
   if [ ${use_zero_pointings} = "no" ] ; then
#      swid_list=`${dal_dump_cmd} | /bin/grep -v ${revno}0000`
      swid_list=`${fdump_cmd} | grep scw | /bin/cut -d'/' -f7 | /bin/sort | /bin/grep -v ${revno}0000`
      error_code=$?
   else
#      swid_list=`${dal_dump_cmd}`
      swid_list=`${fdump_cmd} | grep scw | /bin/cut -d'/' -f7 | /bin/sort`
      error_code=$?
   fi
