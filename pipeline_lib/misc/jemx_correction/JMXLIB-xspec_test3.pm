...
#	Then, for each "jmx${jemxnum}_sky_ima.fits":
	...
	my $format  = "%-20s %20s %20s %20s %20s %20s\n";
	my $format2 = "%-20s %20s %20s %20s %20s %20s\n";
	open (OUTPUT, "> xspec_results" );
	printf OUTPUT $format, "FILENAME", "PhoIndex", "norm", "Red chi-sq", "degrees freedom", "Null hypothesis prob";
...
#		"par_outmode"     => "2",
	...
	printf OUTPUT $format2, "spectrum.fits" ,
		...
		(split /\s+/,(grep /Reduced chi-squared/, @result)[-1])[6],
