...
sub CopyGTIExtension {			
	...
	#
	#	maybe should just check the ALL after evts_pick and if no events there then remove the GTI?
	#
...
#	FIX - this may be a bit of a problem.  Do I want to copy the empty extensions?
	if ( $rows <= 0 ) {
	...
	my $gtidol = &ISDCPipeline::FindDOL ( $grpdol, "$gti_struct-IDX" );	#	pretty much has to be an index
#	$gtidol = &ISDCPipeline::FindDOL ( $grpdol, "$gti_struct" ) unless ( $gtidol );
#	&Error ("No GTI DOL found!?") unless ( $gtidol );		#	FindDOL will error out if not found
	...
	#Log_1  : Selection [GTI_NAME == 'SC'] (1 members.) NumLog : 1
	#Log_0  : /isdc/integration/isdc_int/sw/dev/prod/opus/nrtscw/unit_test/outref/scw/0025/002500010010.000/intl_osm_gti.fits[INTL-GNRL-GTI,5,BINTABLE]
	...
	( $gtifile ) = ( $gtidol =~ /^(.*)\[.*$/ );		#	grab only the filename
	...
	&ISDCLIB::QuickDalCopy   ( "$gtidol", "$targetfile", "" );	#	060113
...
sub Fin {
	...
	my $scwid     = $att{scwid};
	my $proc      = $att{proc};
	my $OBSDIR    = $att{OBSDIR};		#	$ENV{REP_BASE_PROD}/obs_picsit/0024.000/ssip_002400050010
...
#	my $scwdirbase = "$ENV{REP_BASE_PROD}/scw/$revno/$scwid";
	...
	#	only need to do this for the first run instrument
	unless ( -e "$newscwdir/swg.fits" ) {

		#	I need to change this glob to be more specific, but for now I'll just let it cause me grief
	...
	foreach my $inst ("ibis","jmx1","jmx2") {
		...
		$struct =~ s/sc/intl/;					#	don't really need this line as only doing ibis and jmx
		$struct = uc($struct);
		$struct =~ s/(OMC|SPI)/$1\./;			#	don't really need this line as only doing ibis and jmx
	...
	my $obsscwdir = glob ( "$OBSDIR/scw/0*" );	#	since only called from consssa/cssfin.pl, will only return 1 directory
	...
	chdir ( "$newscwdir/" ) or die "Could not chdir $newscwdir";	#	probably already here
	...
	#	save this for almost last so it includes almost everything
...
sub IBISFIN { 
	...
	&ISDCLIB::QuickClean     ( @fitslist );				#	delete old products
	&ISDCLIB::QuickDalClean  ( "swg.fits" );				#	shouldn't need cleaned unless a rerun
	&ISDCLIB::QuickDalDetach ( "swg.fits", "IBIS-GNRL-GTI*", "yes" );		#	Should I delete here?
	...
	#	if we decide to redo PICSIT, I may want to create a new structure here or something.
	...
	#	remove the original ISGR-DEAD-SCP and attach the new one
...
sub AddOldIBISGTIs {
	...
	foreach my $gti_name ( "MERGED_PICSIT", "PICSIT_ISOC", "P_SGLE_DATA_GAPS", "P_MULE_DATA_GAPS", "C_SGLE_DATA_GAPS", "C_MULE_DATA_GAPS"  ) {
		#	Why don't I use CopyGTIExtension here?  Cause it does a bunch of checks.
		#	CopyGTIExtension ( $grpdol, $gti_struct, $gti_name, $targetfile )
		#		&CorLIB::CopyGTIExtension ( "$oldscwdir/swg.fits[1]", "IBIS-GNRL-GTI", $gti_name, "ibis_gti.fits" )
		...
		chomp ( my $count = `dal_list ibis_gti.fits[1] | grep TABLE  | wc -l` );		#	$count += 2;
		$count += 2;	#  +2 to account for offset of [0] and [1]
		&ISDCPipeline::PipelineStep(
			...
			"par_security"  => "0",
			"par_update"    => "1",
			"par_stamp"     => "1",								#	after testing maybe change this to 0
			);

		#	FIX ? idx_add will correctly copy the GTI_NAME, but the MEMBER_URI_TYPE is URL instead of NULL like the rest
	}
...
sub GTI_Merge {
	...
	if ( ( $INST =~ /IBIS/ ) && ( $name =~ /PICSIT/ ) ) {
		...
		$pars{"par_JEMX${1}_Names"} = "OSM1 DATA_GAPS";
	} elsif ( $INST =~ /SPI/ ) {
