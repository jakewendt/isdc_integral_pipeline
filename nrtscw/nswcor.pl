...
&ISDCPipeline::PipelineStep(
	...
	"par_AttStability" => "0.5",
	"par_Instrument"   => "SC",			#	Jake added 040109 - SPR 3411 - default = "SC"
	"par_AttStability_Z" => "3.0",		#	060529 - Jake - SCREW 1872 added from gti_attitude 1.5
	);
...
&ISDCPipeline::PipelineStep(
	...
	"par_FindAllIbisGaps"   => "no",
	"par_RemoveWrongGTIs"   => "yes",	#	041018 - Jake - gti_data_gaps-2.4
	);
...
sub ibisAnalysis {
	...
	#	040603 - Jake - As per SCREW 1416, do not create a MERGED_COMPTON and clearly don't copy it to compton_events.fits
...
sub jemxAnalysis {												
	my $num   = $_[0];
	#	f(a)=b; f(b)=a; f(x)=(a+b)-x
	my $other = 3 - $num ;									
	#	Yes, I know.  Clever bad.  Simple good.  But I'm keeping this one.
	my $instr = "JMX"."$num";
	...
	&ISDCPipeline::PipelineStep(
		...
		"par_outputExists"       => "YES",					
		"par_j_cor_gain_gainFac" => "",		#	060925 - Jake - j_correction 4.6
		"par_subDir"             => "",
		"par_randPos" => "n",				#	070620 - Jake 
		);
...
sub spiAnalysis {
	...
	&ISDCPipeline::PipelineStep(
		...
		"par_randomise" => "yes",
		"par_clobber"   => "yes",
		"par_overwriteALL" => "no",		#	060315 - Jake - added this as default for v-1.8
		);
