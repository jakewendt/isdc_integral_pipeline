...
I<adpst.pl> - Run from within B<OPUS>.  This is the first step of a three stage pipeline which does Auxiliary Data Preparation.  The second step is B<adp.pl> and the last step is B<adpfin.pl>.

=head1 DESCRIPTION

I<adpst.pl> - Run from within B<OPUS>.  This is the first step of the three step ADP pipeline.  The purpose of this step is to trigger on the existance of a file and create an B<OPUS> observation entry which will be used to trigger the other steps of the pipeline.  All triggering is done from the B<ADP_INPUT> directory and the triggering is done by B<OPUS>.  By the time the script is running the file already exists in the input directory.  The main job of the script is to create and open the log file for the processing of this ADP file and to create an Observation Status File which enters the file into the B<OPUS> processing stream.
...
This is the location of the auxiliary data repository and is usally set to the B<rii_aux> entry in the path file.
...
if (!(-e $orgdir)) {
	#	061211 - Jake - convert all dies to ISDCLIB::Error or ISDCLIB::DoOrDie
	#	`$mymkdir -p $orgdir`;
	#	die "*******     ERROR:  cannot mkdir $orgdir" if ($?);
	&ISDCLIB::DoOrDie ( "$mymkdir -p $orgdir" );
}
...
if (!(-e $adpdir)) {
	...
	&ISDCLIB::DoOrDie ( "$mymkdir -p $adpdir" );
}
...
This is a pad file which has a 2 digit AO number in the file name.  This sets the log file directory to the proper AO directory and the type to "pad".

=item B<iop_([0-9]{2}).*>

This is an iop file which has a 2 digit AO number in the file name.  This action sets the log file directory to the proper AO directory and the type to "iop".

=item B<ocs_([0-9]{2}).*>

This is a ocs file which has a 2 digit AO number in the file name.  The original name of the file, ending in ".fits", is just barely too long for an OPUS observation, so the OSF name is set to "_fi" instead.  This action sets the log file directory to an "ocs" subdirectory of the proper AO directory.  It sets the type to "ocs".  
...
This is a pod file which has a 4 digit revolution number in the file name.  This action sets the log file directory to the proper revolution directory and the type to "pod".

=item B<opp_([0-9]){4})>

This is an opp file which has a 4 digit revolution number in the file name.  This action sets the log file directory to the proper revolution directory and the type to "opp".
...
This is a PAF file which has a 4 digit revolution number in the file name.  This action sets the log file directory to the "logs" subdirectory of the proper revolution directory.  It sets the type to "paf".

=item B<([0-9]{4})_([0-9]{4})\.(ASF|AHF)>

This is either a ASF or an AHF file which has a 4 digit revolution number in the file name.  This action sets the log file directory to the "logs" subdirectory of the the proper revolution directory. If it is an ASF file, the logfile directory is in yet another subdirectory named "asf".  The type is set to either "asf" or "ahf".  
...
=cut
	...
	if ($dataset =~ /([0-9]{4})_([0-9]{2,4})\.(PAF|ASF|AHF)/) {
		...
		if ( $type == "ahf" ) {	#	060210 - Jake - SPR 4392
			...
			&ISDCLIB::Error ( "$adpdir/$rev.000/attitude_historic.fit* already exists!" )
...
This is the orbita file which was renamed by I<adpmon.pl> to orbita.orbita.  The file is renamed orbita_YYYYMMDD and the log file is set to the top level "adp" directory for now.  It will be moved later;  see adp.pl.  The type is set to "orb".  
...
This is the revno file which was renamed by I<adpmon.pl> to revno.revno.  Now it is renamed to revno_YYYYMMDD and the log file is set to the the adp/ref/revno/ subdirectory.  The type is set to "rev".
...
This is a TSF file which has a 4 digit revolution number in the file name.  This action sets the log file directory to the "logs" subdir of the proper revolution directory.  The type is "tsf".  The long name of the original TSF file is shortened to be simply TSF_RRRR_VVVV.INT.  
...
This is a OLF file.  It is not possible to derive the proper directory from the file name since the revolution number is not contained in the file name.  For now this log file is put in the adp directory and it will be moved later.  The type is set to "olf".
...
If none of the above types matches the input, the file is moved to the "odd" subdirectory of the input directory, and an alert is sent.  
...
if (!-e $logfiledir) {
	...
	&ISDCLIB::DoOrDie ( "$mymkdir -p $logfiledir" );
}
...
if ( ($alert) || (-e "$logfilename") || (@prev_osfs) ) {
	...
	if ($alert) {
		...
		&ISDCLIB::Error ( "cannot mkdir $ENV{WORKDIR}/adpst:  @result" ) if ($retval);
		#	061211 - Jake - convert all dies to ISDCLIB::Error or ISDCLIB::DoOrDie
		#	chdir "$ENV{WORKDIR}/adpst" or die "*******     ERROR:  cannot chdir $ENV{WORKDIR}/adpst";
		chdir "$ENV{WORKDIR}/adpst" or &ISDCLIB::Error ( "cannot chdir $ENV{WORKDIR}/adpst" );
		...
		&ISDCLIB::Error ( "cannot copy alert: @result" ) if ($retval);
...
Check if rev has already been triggered for archive ingest;  should not have any new files tiggered if so!
...
The resource file I<adpst.resource> contains all the file triggers for the B<ADP> pipeline.

=head1 RESTRICTIONS

Any changes in the input file names will require changes in this script since it matches on the file names.  Note that corresponding changes will also have to be made in I<adp.pl> and I<adpmon.pl>.

=head1 REFERENCES

For further information on B<adp.pl> or B<adpfin.pl> please run perldoc on those files, ie, C<perldoc adp.pl>.
